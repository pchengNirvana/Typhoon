  ...
  subroutine read_dat_file &
    ...
    loop_t: do l = 1, nt 
      ...
      if (present(xlat)) then
        read(lun, rec = 0 *zmax+0 +1+(l-1)*tb) ((xlat(i, j, l), i = 1, nx), j = 1, ny)
      end if
      if (present(xlong)) then
        read(lun, rec = 0 *zmax+1 +1+(l-1)*tb) ((xlong(i, j, l), i = 1, nx), j = 1, ny)
      end if
      if (present(q2)) then
        read(lun, rec = 3 *zmax+2 +1+(l-1)*tb) ((q2(i, j, l), i = 1, nx), j = 1, ny)
      end if
      if (present(t2)) then
        read(lun, rec = 3 *zmax+3 +1+(l-1)*tb) ((t2(i, j, l), i = 1, nx), j = 1, ny)
      end if
      if (present(psfc)) then
        read(lun, rec = 3 *zmax+4 +1+(l-1)*tb) ((psfc(i, j, l), i = 1, nx), j = 1, ny)
      end if
      if (present(u10)) then
        read(lun, rec = 3 *zmax+5 +1+(l-1)*tb) ((u10(i, j, l), i = 1, nx), j = 1, ny)
      end if
      if (present(v10)) then
        read(lun, rec = 3 *zmax+6 +1+(l-1)*tb) ((v10(i, j, l), i = 1, nx), j = 1, ny)
      end if
      if (present(hgt)) then
        read(lun, rec = 10*zmax+7 +1+(l-1)*tb) ((hgt(i, j, l), i = 1, nx), j = 1, ny)
      end if
      if (present(tsk)) then
        read(lun, rec = 10*zmax+8 +1+(l-1)*tb) ((tsk(i, j, l), i = 1, nx), j = 1, ny)
      end if
      if (present(rainc)) then
        read(lun, rec = 10*zmax+9 +1+(l-1)*tb) ((rainc(i, j, l), i = 1, nx), j = 1, ny)
      end if
      if (present(rainnc)) then
        read(lun, rec = 10*zmax+10+1+(l-1)*tb) ((rainnc(i, j, l), i = 1, nx), j = 1, ny)
      end if
      if (present(hfx)) then
        read(lun, rec = 10*zmax+11+1+(l-1)*tb) ((hfx(i, j, l), i = 1, nx), j = 1, ny)
      end if
      if (present(qfx)) then
        read(lun, rec = 10*zmax+12+1+(l-1)*tb) ((qfx(i, j, l), i = 1, nx), j = 1, ny)
      end if
      if (present(lh)) then
        read(lun, rec = 10*zmax+13+1+(l-1)*tb) ((lh(i, j, l), i = 1, nx), j = 1, ny)
      end if
      if (present(sst)) then
        read(lun, rec = 10*zmax+14+1+(l-1)*tb) ((sst(i, j, l), i = 1, nx), j = 1, ny)
      end if
      if (present(slp)) then
        read(lun, rec = 14*zmax+15+1+(l-1)*tb) ((slp(i, j, l), i = 1, nx), j = 1, ny)
      end if
      if (present(maxdbz)) then
        read(lun, rec = 15*zmax+16+1+(l-1)*tb) ((maxdbz(i, j, l), i = 1, nx), j = 1, ny)
      end if

      ! read 3D variables
      loop_z: do k = 1, nz
        if (present(u)) then
          read(lun, rec = 0 *zmax+2 +k+(l-1)*tb) ((u(i, j, k, l), i = 1, nx), j = 1, ny)
        end if
        if (present(v)) then
          read(lun, rec = 1 *zmax+2 +k+(l-1)*tb) ((v(i, j, k, l), i = 1, nx), j = 1, ny)
        end if
        if (present(w)) then
          read(lun, rec = 2 *zmax+2 +k+(l-1)*tb) ((w(i, j, k, l), i = 1, nx), j = 1, ny)
        end if
        if (present(qv)) then
          read(lun, rec = 3 *zmax+7 +k+(l-1)*tb) ((qv(i, j, k, l), i = 1, nx), j = 1, ny)
        end if
        if (present(qc)) then
          read(lun, rec = 4 *zmax+7 +k+(l-1)*tb) ((qc(i, j, k, l), i = 1, nx), j = 1, ny)
        end if
        if (present(qr)) then
          read(lun, rec = 5 *zmax+7 +k+(l-1)*tb) ((qr(i, j, k, l), i = 1, nx), j = 1, ny)
        end if
        if (present(qi)) then
          read(lun, rec = 6 *zmax+7 +k+(l-1)*tb) ((qi(i, j, k, l), i = 1, nx), j = 1, ny)
        end if
        if (present(qs)) then
          read(lun, rec = 7 *zmax+7 +k+(l-1)*tb) ((qs(i, j, k, l), i = 1, nx), j = 1, ny)
        end if
        if (present(qg)) then
          read(lun, rec = 8 *zmax+7 +k+(l-1)*tb) ((qg(i, j, k, l), i = 1, nx), j = 1, ny)
        end if
        if (present(hdiabatic)) then
          read(lun, rec = 9 *zmax+7 +k+(l-1)*tb) ((hdiabatic(i, j, k, l), i = 1, nx), j = 1, ny)
        end if
        if (present(height)) then
          read(lun, rec = 10*zmax+15+k+(l-1)*tb) ((height(i, j, k, l), i = 1, nx), j = 1, ny)
        end if
        if (present(tk)) then
          read(lun, rec = 11*zmax+15+k+(l-1)*tb) ((tk(i, j, k, l), i = 1, nx), j = 1, ny)
        end if
        if (present(theta)) then
          read(lun, rec = 12*zmax+15+k+(l-1)*tb) ((theta(i, j, k, l), i = 1, nx), j = 1, ny)
        end if
        if (present(rh)) then
          read(lun, rec = 13*zmax+15+k+(l-1)*tb) ((rh(i, j, k, l), i = 1, nx), j = 1, ny)
        end if
        if (present(dbz)) then
          read(lun, rec = 14*zmax+16+k+(l-1)*tb) ((dbz(i, j, k, l), i = 1, nx), j = 1, ny)
        end if
      end do loop_z
    end do loop_t
